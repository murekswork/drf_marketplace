...
class ArticleModelTestCase(TestCase):
    ...
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='test', email='test@mail.com')
        self.shop = Shop.objects.create(user=self.user, title='title', description='description')
        self.product = Product.objects.create(shop=self.shop, title='title', public=True, content='content', quantity=5)
        self.order = Order.objects.create(product=self.product, count=3, user=self.user)
        self.article1 = Article.objects.create(user=self.user, title='title', article_content='article content',
                                               published=True, mark=5, order=self.order, product=self.product)
    ...
    def test_article_model_when_second_article_create_then_product_mark_changes(self):
        self.article2 = Article.objects.create(user=self.user, title='title', article_content='article content',
                                               published=True, mark=1, order=self.order, product=self.product)

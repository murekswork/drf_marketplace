...
class OrderModelSaveMethodTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(shop=self.shop, title='title', public=True, content='content',
                                              quantity=5)
        self.order = Order.objects.create(product=self.product, count=3, user=self.user)

    def test_order_model_when_create_not_paid_then_product_sales_count_not_increases(self):
    ...
    def test_order_model_when_create_and_paid_second_then_product_sales_count_increases(self):
        self.order2 = Order.objects.create(product=self.product, count=3, user=self.user)
        self.order.payment_status = True
        self.order.save()

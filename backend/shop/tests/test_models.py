...
class ShopModelTestCase(TestCase):
    ...
    def test_create_shop(self):
        shop = Shop.objects.create(user=self.user, title='test_shop', description='test_shop_description')
        shop.save()
...
class TestShopStaffGroupModelTestCase(TestCase):
    ...
    def setUp(self):
        self.shop_owner = get_user_model().objects.create(username='owner', email='owner@email.com')
        self.shop = Shop.objects.create(title='Test Shop', description='Test Shop', user=self.shop_owner)
        self.shop_staff_group = ShopStaffGroup.objects.create(group_name='Product Owner')

        self.shop_staff_group.permissions.set(Permission.objects.filter(content_type__app_label='shop'))

    def test_shop_staff_group_permissions(self):
        self.assertIn(Permission.objects.get(codename='create_shop_product'),
                      self.shop_staff_group.permissions.select_related())
        self.assertNotIn(Permission.objects.get(codename='add_product'),
                         self.shop_staff_group.permissions.select_related())
...
class TestShopManagerModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.product_manager_group = ShopStaffGroup.objects.create(group_name='Product Manager')
        self.product_manager_group.permissions.set(Permission.objects.filter(codename__in=['create_shop_product',
                                                                                           'update_shop_product',
                                                                                           'delete_shop_product']))
        self.user_manager = get_user_model().objects.create(username='product_manager', email='dummy@mail.com')
        self.shop_manager = ShopManager(user=self.user_manager, title='Product Manager Full',
                                        group=self.product_manager_group, shop=self.shop)
    ...
    def test_shop_owner_has_all_permissions(self):
        for permission in [
            'create_shop_product', 'update_shop_product', 'delete_shop_product',
            'create_shop_sales', 'update_shop_sales', 'delete_shop_sales',
            'manage_shop_data', 'can_manage_shop_managers',
        ]:
            self.assertTrue(
                self.shop.managers.through.objects.filter(user=self.shop_owner)[0].has_permission(permission))

...
class TestManagerPermissions(TestCase):
    ...
    def setUp(self):
        ...
        self.product_manager = User.objects.create_user(username='product_manager', email='product_naager@email.com')
        for perm in [Permission.objects.filter(codename__in=['create_shop_product',
                                                             'update_shop_product',
                                                             'delete_shop_product']).all()]:
        ...
        for perm in [Permission.objects.filter(codename__in=['create_shop_sales',
                                                             'update_shop_sales',
                                                             'delete_shop_sales']).all()]:
            self.sales_manager_group.permissions.add(perm)
        self.sales_manager_group.save()
        self.sales_manager = User.objects.create_user(username='sales_manager', email='salesmanager@email.com')
        ...
        self.product = Product.objects.create(public=True, title='prodict', content='Content', price=15, shop=self.shop)
